...
class MountSourcePolicy(allowedPrefixes: List<Path>) {
    private val allowedPrefixes = allowedPrefixes.map {
    ...
    fun validate(source: Path): Path {
        ...
        require(allowedPrefixes.any { isInsideAllowedPrefix(realSource, it) }) {
            "Mount source is not allowed: $source"
        }
        return realSource
    }
}

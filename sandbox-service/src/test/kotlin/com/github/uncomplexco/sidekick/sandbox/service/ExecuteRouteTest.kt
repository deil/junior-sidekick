...
class ExecuteRouteTest {
    @Test
    fun `execute rejects missing bearer token`() = testApplication {
        // Arrange
    ...
    @Test
    fun `execute rejects invalid bearer token`() = testApplication {
        // Arrange
        application { sandboxServiceModule(testConfig()) }

        // Act
        val response =
            client.post("/api/execute") { header(HttpHeaders.Authorization, "Bearer wrong") }
    ...
    @Test
    fun `execute maps valid request to sandbox executor`() = testApplication {
        // Arrange
    ...
    @Test
    fun `execute rejects mount source outside allowed prefixes`() = testApplication {
        // Arrange
        val allowed = Files.createTempDirectory("sandbox-service-allowed")
    ...
    @Test
    fun `execute rejects invalid mount mode`() = testApplication {
        // Arrange
        val allowed = Files.createTempDirectory("sandbox-service-allowed")
        application { sandboxServiceModule(testConfig(allowedPrefixes = listOf(allowed))) }
    ...
    private fun testConfig(
        allowedPrefixes: List<java.nio.file.Path> = emptyList()

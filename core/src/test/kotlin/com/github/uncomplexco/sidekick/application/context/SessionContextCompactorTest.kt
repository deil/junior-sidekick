...
class SessionContextCompactorTest {
    ...
    @Test
    fun `does not compact when estimated context is below trigger`() = runBlocking {
        // Arrange
    ...
    @Test
    fun `compacts oldest messages and keeps latest live messages`() = runBlocking {
        // Arrange
        val summarizer = RecordingSummarizer()
        val compactor = compactor(summarizer)
        val state =
    ...
    @Test
    fun `compacts once and preserves minimum live messages`() = runBlocking {
        // Arrange
        val summarizer = RecordingSummarizer()
        val compactor = compactor(summarizer)
        val state =
    ...
    @Test
    fun `records assistant message count in compaction`() = runBlocking {
        // Arrange
        val summarizer = RecordingSummarizer()
        val compactor = compactor(summarizer)
        val state =
                ...
                messages =
                    (1..40).map {
                        message(
                            ...
                            role =
                                if (it % 3 == 0) SessionMessageRole.ASSISTANT
                                else SessionMessageRole.USER,
                            text = longText(),
                        )
                    },
            )
    ...
    private fun compactor(summarizer: RecordingSummarizer): SessionContextCompactor =
        SessionContextCompactor(summarizer)

...
class TurnPromptBuilderSkillsTest {
    ...
    fun `renders model invocable skills catalog`() {
        ...
        assertTrue(prompt.contains("call the activateSkill tool"), prompt)
    ...
    @Test
    fun `does not render skills disabled for model invocation`() {
        // Arrange
        val prompt =
            builder(
                SkillCatalog(
                    listOf(
                        skill("disabled-skill", "User-only skill.", disableModelInvocation = true),
                    ),
                ),
            ).buildSessionTurnPrompt(message(), context())
        ...
        assertFalse(prompt.contains("disabled-skill"), prompt)
    }
    ...
    private fun message(): com.github.uncomplexco.sidekick.application.conversation.SessionMessage =

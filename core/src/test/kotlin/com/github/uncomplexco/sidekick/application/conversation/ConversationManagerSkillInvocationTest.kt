...
class ConversationManagerSkillInvocationTest {
    ...
    @Test
    fun `persists materialized explicit skill invocation on current message`() = runBlocking {
        // Arrange
    ...
    @Test
    fun `does not materialize explicit skill invocation for seeded history`() = runBlocking {
        // Arrange
        val conversationId = ConversationId("C123", "1700000000.000")
        val store = store()
        val manager = manager(store)
        ...
        manager.recordIncomingMessages(
            ...
            messages =
                listOf(message(id = "current", text = "please review this", createdAtMs = 2)),
            files = emptyList(),
        )

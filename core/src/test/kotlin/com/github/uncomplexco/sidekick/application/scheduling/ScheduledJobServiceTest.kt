...
class ScheduledJobServiceTest {
    @Test
    fun `creates normalized channel job`() = runBlocking {
        // Arrange
    ...
    @Test
    fun `enforces unique names and active limit`() = runBlocking {
        // Arrange
        val service = ScheduledJobService(TestScheduledJobStore())
        repeat(ACTIVE_JOB_LIMIT) { index -> service.create("C123", command(name = "job $index")) }
    ...
    @Test
    fun `rejected creation does not consume id`() = runBlocking {
        // Arrange
        val service = ScheduledJobService(TestScheduledJobStore())
    ...
    @Test
    fun `updates mutable fields and preserves runtime fields`() = runBlocking {
        // Arrange
        val store = TestScheduledJobStore()
        val service = ScheduledJobService(store)
    ...
    @Test
    fun `claims only matching minute and enforces cooldown`() = runBlocking {
        // Arrange
        val store = TestScheduledJobStore()
        val service = ScheduledJobService(store)
        val job =
            service.create("C123", command(schedule = "0 9 * * *", timezone = "Europe/London"))
    ...
    @Test
    fun `cooldown ignores seconds at minute precision`() = runBlocking {
        // Arrange
        val store = TestScheduledJobStore()
        val service = ScheduledJobService(store)
    ...
    @Test
    fun `does not catch up missed occurrences`() = runBlocking {
        // Arrange
        val service = ScheduledJobService(TestScheduledJobStore())

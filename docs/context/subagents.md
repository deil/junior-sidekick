...
- `subagent_type` - agent definition name, defaulting to `general`.
...
Subagent system prompts come from the agents catalog. For a `subagent_type` named `general`, Sidekick loads:

```text
agents/general.md
```

Built-in definitions live as application resources under `tools/src/main/resources/agents/`. Agent definition files may contain YAML frontmatter; Sidekick strips the first frontmatter block before using the Markdown body as the system prompt.
...
- `tools/src/main/kotlin/com/github/uncomplexco/sidekick/application/tools/subagents/AgentDefinitionCatalog.kt` - built-in agent definition loading and frontmatter stripping.

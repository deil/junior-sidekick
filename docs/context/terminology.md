...
| **Session** | An interactive conversation with the agent that runs through a chat platform and contains one or more turns. | Chat, thread, channel |
| **Turn** | One user message and the agent work performed in response to it. A turn is scoped within a session and ends when the agent replies, declines to reply, or completes another requested action. | Thread, session, request |

...
class SlackFileToolsTest {
    ...
    fun `supports html markdown and text files`() {
        ...
        assertTrue(
            isSupportedSlackTextFile(file(mimetype = "text/markdown", filetype = "markdown"))
        )
        assertTrue(isSupportedSlackTextFile(file(mimetype = "text/plain", name = "note.txt")))

        assertFalse(
            isSupportedSlackTextFile(
                file(mimetype = "application/pdf", filetype = "pdf", name = "note.pdf")
            )
        )
    }
    ...
    fun `read returns virtual session path for attached file`() {
        ...
        val file =
            file(
                ...
                localPath = "/data/session/F123-note.md",
            )

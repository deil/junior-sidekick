...
class SlackReactionToolsTest {
    ...
    fun `normalizes optional surrounding colons and lowercase`() {
        ...
        assertEquals(
            ...
            normalizeSlackReactionEmoji(":ThumbsUp::skin-tone-6:"),
        )
    }

    @Test
    fun `rejects blank emoji alias`() {
        assertThrows<IllegalArgumentException> { normalizeSlackReactionEmoji("::") }
    }

    @Test
    fun `rejects emoji alias that does not match slack reaction regex`() {
        assertThrows<IllegalArgumentException> { normalizeSlackReactionEmoji("white check mark") }
        assertThrows<IllegalArgumentException> {
            normalizeSlackReactionEmoji("thumbsup::skin-tone-7")
        }
        assertThrows<IllegalArgumentException> { normalizeSlackReactionEmoji("thumbsup::foo") }
    }
}

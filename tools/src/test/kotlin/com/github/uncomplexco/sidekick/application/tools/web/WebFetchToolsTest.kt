...
class WebFetchToolsTest {
    ...
    @Test
    fun `fetches text from http url`() = withServer { server ->
        server.handler("/plain") { exchange -> exchange.respond("hello", "text/plain") }
    ...
    @Test
    fun `follows redirects`() = withServer { server ->
        server.handler("/redirect") { exchange ->
            exchange.responseHeaders.add("Location", "/target")
            exchange.sendResponseHeaders(302, -1)
            exchange.close()
        }
        server.handler("/target") { exchange -> exchange.respond("redirected", "text/plain") }

        assertEquals(
            "redirected",
            webFetchTools().webFetch("${server.url}/redirect", format = "text").output,
        )
    }

    @Test
    fun `rejects images and oversized responses`() = withServer { server ->
        server.handler("/image") { exchange -> exchange.respond("png", "image/png") }
        server.handler("/big") { exchange ->
            exchange.responseHeaders.add("Content-Type", "text/plain")
            exchange.responseHeaders.add(
                "Content-Length",
                (MAX_WEB_FETCH_RESPONSE_BYTES + 1).toString(),
            )
    ...
    @Test
    fun `retries cloudflare challenge with agent user agent`() = withServer { server ->
        ...
        assertEquals(
            "ok",
            webFetchTools().webFetch("${server.url}/challenge", format = "text").output,
        )

...
class ScheduledJobTools(
    ...
    fun createScheduledJob(
        @LLMDescription("Required unique human-readable job name") name: String,
        @LLMDescription("Optional job description") description: String? = null,
        @LLMDescription("Required five-field cron: minute hour day-of-month month day-of-week")
        schedule: String,
        @LLMDescription("Required IANA timezone") timezone: String,
        @LLMDescription("Required instruction executed on every scheduled run") prompt: String,
        @LLMDescription("Whether the job is active. Defaults to true") enabled: Boolean = true,
    ): ScheduledJobResult = asToolCall {
    ...
    fun updateScheduledJob(
        @LLMDescription("ID of the job to update") job_id: Int,
        @LLMDescription("New unique human-readable name") name: String? = null,
        ...
        @LLMDescription("New IANA timezone") timezone: String? = null,
        @LLMDescription("New instruction executed on every scheduled run") prompt: String? = null,
        @LLMDescription("Set false to pause or true to resume") enabled: Boolean? = null,
    ): ScheduledJobResult = asToolCall {
        val job = runBlocking {
            jobs.update(
                channelId,
    ...
    fun deleteScheduledJob(
        @LLMDescription("ID of the job to delete") job_id: Int
    ): DeletedScheduledJobResult = asToolCall {
...
@Serializable data class ScheduledJobResult(val job: ScheduledJobView)

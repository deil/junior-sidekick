...
class SlackFileMappingTest {
    ...
    fun `uses directly attached files when present`() {
        val files =
            incomingChatFiles(
                ...
                attachments =
                    listOf(Attachment.builder().files(listOf(slackFile("F_FORWARDED"))).build()),
            )
    ...
    @Test
    fun `ingested file path is session based`() = runBlocking {
        ...
        try {
            ...
            val file =
                incomingChatFiles(
                        files =
                            listOf(slackFile("F1", "http://127.0.0.1:${server.address.port}/F1")),
                        attachments = emptyList(),
                    )
                    .single()
            ...
            assertTrue(
                Files.exists(
                    conversationId.folder(dir, ChatPlatform.SLACK).resolve("attachments/F1-F1.md")
                )
            )
        } finally {
            server.stop(0)
    ...
    @Test
    fun `ingested images are summarized`() = runBlocking {
        val server = HttpServer.create(InetSocketAddress("127.0.0.1", 0), 0)
        server.createContext("/F1") { exchange ->
        ...
        try {
            ...
            assertEquals(
                conversationId.folder(dir, ChatPlatform.SLACK).resolve("attachments/F1-screen.png"),
                summarizedPath,
            )
        } finally {
            server.stop(0)
